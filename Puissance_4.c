...
static unsigned calcule_nb_jetons_depuis_vers(struct position *pos, int dpl_hrz, int dpl_vrt, char jeton)
{
     ...
     * La fonction s'arrête si un jeton différent ou une case vide rencontrée ou si
...
{
    /*
    * Calcule le nombre de jetons adjacents en vérifant la
   onne courante,
    * de la ligne courante et des deux obliques courantes.
    * Pour ce faire, la fonction calcule_nb_jeton_depuis_vers()
    appelé à
    * plusieurs reprises afin de parcourir la grille suivant la
   ification
    * à effectuer.
    */
...
static int charger_jeu(char *nom, char *jeton, int *njoueur)
{
    ...
    {
        fprintf(stderr, "Impossible de récupérer le joueur et le nombre de joueurs c\n");
        return 0;
    }
...
static int demande_action(int *coup)
{
    ...
    * S'il entre la lettre « Q » ou « q », c'est qu'il souhaite
   tter.
...
static int demande_nb_joueur(void)
{
    ...
    while (1)
    {
        printf("Combien de joueurs prennent part a cette partie (1-2) ? ");
...
static int ia(void)
{
    ...
    * Assigne une valeur pour chaque colonne libre et retourne
   uite le numéro de
    * colonne ayant la plus haute valeur. Dans le cas où
   sieurs valeurs égales sont
    * générées, un numéro de colonne est « choisi au hasard »
   mi celles-ci.
    */
...
{
    /*
    * Retourne un nombre pseudo-aléatoire compris entre zéro
   lus et un exclus.
    */
...
int nb_aleatoire_entre(int min, int max)
{
    ...
    * Retourne un nombre pseudo-aléatoire entre `min` et `max`
   lus.
    */
...
{
    /*
    * Vérifie que la position fournie est bien comprise dans la
   lle.
    */
...
{
    /*
    * Détermine s'il y a lieu de continuer le jeu ou s'il doit
    continuer
    * arrêté parce qu'un joueur a gagné ou que la grille est
    complète.
    */
...
int main(void)
{
    ...
    initialise_grille();
    affiche_grille();
    printf("\nPour sauvegarder une partie (appuyer sur 's' minuscule)\nPour charger une partie(appuyer sur 'c' minusucle)\nPour quitter la partie(appuyer sur 'q' minuscule)\n\n");
